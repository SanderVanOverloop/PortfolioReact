        ...
        <div className="overflow-y-auto max-h-[500px]">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {Object.entries(categories).map(([category, skills]) => (
                    <div
                        key={category}
                        className="min-w-[250px] bg-gray-200 border-gray-400 border backdrop-blur-sm p-5 rounded-2xl shadow-md"
                    >
                        ...
                        <ul className="grid grid-cols-1 gap-3 text-center">
                            {skills.map((skill) => (
                                <li
                                    key={skill}
                                    className="p-2 rounded-lg font-medium shadow-sm hover:shadow-md transition-transform hover:scale-105 bg-gray-300 text-gray-900"
                                >
                                    {skill}
                                </li>
                            ))}
    ...
    const renderPersonalSkills = (skills: typeof personalSkills) => (
        <div className="flex flex-col gap-8 items-center">
            {Object.entries(skills).map(([category, items]) => (
                <div
                    key={category}
                    className="w-full max-w-3xl p-6 rounded-xl shadow-md bg-gray-100 border border-gray-300 text-gray-900"
                >
                    ...
                    <ul className="grid grid-cols-2 md:grid-cols-3 gap-4">
                        {items.map((skill) => (
                            <li
                                key={skill}
                                className="p-3 rounded-lg text-center text-sm font-medium shadow-sm transition-transform hover:scale-105 bg-gray-200 text-gray-800 border border-gray-400"
                            >
                                {skill}
                            </li>
                        ))}
        ...
        <>
            ...
            <main className="min-h-screen px-6 py-12 bg-white text-gray-900">
                ...
                <section className="max-w-6xl mx-auto mt-16 px-4">
                    <div className="rounded-2xl p-6 md:p-8 shadow-lg transition-all bg-gray-100 border-gray-300">
